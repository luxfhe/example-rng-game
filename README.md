...
This repo is an example of the RNG functions released as part of the luxfhe Nitrogen testnet. Take a look at the [randomness docs (TODO:FIXLINK)](https://) for more information.
...
This example was built using the `luxfhe-hardhat-example` repo found [here](https://github.com/luxfheProtocol/luxfhe-hardhat-example). See the readme below for setting up and interacting with this repo.

---

`luxfhe-hardhat-example:README.md`
...
Once the file exists, let's run a Localluxfhe instance:
...
If Localluxfhe doesn't start this could indicate an error with docker. Please verify that docker is running correctly using the `docker run hello-world` command, which should run a basic container and verify that everything is plugged in.
